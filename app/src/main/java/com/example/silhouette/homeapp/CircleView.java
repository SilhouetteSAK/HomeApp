...
/**
 * Created by Silhouette on 2018/4/26.
 */
...
public class CircleView extends View {
    ...
    private int currentDegree = 280;//当前度数（除360可求百分比）
    @SuppressLint("NewApi")
    ...
    private static final int COLOR1 = Color.parseColor("#104E8B");
    private static final int COLOR2 = Color.parseColor("#CD2626");
    private static final int COLOR3 = Color.parseColor("#151B3B");

    private final static int strokeWidth = 120;//画笔大小
...
//      handler.postDelayed(runnable, 500);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < currentDegree; i++) {
            if (!isGradual) {
                if (i < 180) {
                    paint.setColor(COLOR1);//右半边颜色
                } else {
                    paint.setColor(COLOR2);//所半边颜色
                }
            } else {
                Integer color = (Integer) argbEvaluator.evaluate(i / 360f, COLOR1, COLOR2);//颜色插值器（level 11以上才可以用）
                paint.setColor(color);
            }
            if (i % 2 == 0) {
                canvas.drawArc(oval, -90 + i, 2.5f, false, paint); // 绘制圆弧 1.35f是每个色块宽度
            }
        }
        //绘制白色的底部背景
//          if(currentDegree < 360)
//          {
//              paint.setColor(COLOR3);//右半边颜色
//              for(int j = currentDegree; j < 360; j++)
//              {
//                  if (j % 2 == 0)
//                  {
//                  canvas.drawArc(oval, -90 + j, 2.5f, false, paint); // 绘制圆弧 1.35f是每个色块宽度
//                  }
//              }
//          }
    }
